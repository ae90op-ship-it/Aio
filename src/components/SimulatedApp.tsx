...
import { ArrowRight, Settings, Copy, CheckCircle2, Save } from "lucide-react";
...
export function SimulatedApp({
  ...
  const handleSimulate = () => {
    ...
    setTimeout(() => {
      ...
      if (appId === "calculator") {
        ...
        setOutputData(
          `### Simulated Output: ${app.name[lang]}\n\n**Processed Input:**\n\n${inputData}\n\n---\n\n*Status: SUCCESS*\n\n*Time: ${new Date().toLocaleTimeString()}*`,
        );
      }
    }, 800);
  };
  ...
  const handleSaveApp = () => {
    if (onSaveNote) {
      onSaveNote(app.name[lang], inputData + "\n\n" + outputData);
    }
  };
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="text-center">
            <h2 className="text-xl text-neutral-600 dark:text-neutral-300 font-medium mb-2">
          ...
          <div className="bg-white dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700/50 rounded-2xl p-6 shadow-sm space-y-4">
            <textarea
              ...
              placeholder={
                appId === "calculator"
                  ? "Enter equation (e.g., 2 + 2, det([[1, 2], [3, 4]]))..."
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-12">
            <button
              onClick={onBackToNotes}
              className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-700 hover:text-neutral-900 dark:hover:text-white px-6 py-3 rounded-xl transition-all border border-neutral-200 dark:border-neutral-700 font-medium"
            >
              {lang === "ar"
                ? "↩️ عودة للملاحظات"
                : lang === "en"
