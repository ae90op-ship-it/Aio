...
export function ClockApp({ lang, theme, onBack }: Props) {
  ...
  // Timer state
  const [timerDuration, setTimerDuration] = useState(25 * 60); // 25 mins
          ...
          {activeTab === 'clock' && (
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
              ...
              <div className="text-xl text-blue-500 font-medium">
                {time.toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'stopwatch' && (
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center w-full">
              ...
              <div className="flex gap-6">
                ...
                >
                  {swRunning ? <Square className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 ml-1 fill-current" />}
                </button>
                <button 
          ...
          {activeTab === 'timer' && (
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center w-full">
              ...
              {!timerRunning && timerDuration === 0 && (
                <div className="text-red-500 mb-8 font-bold animate-bounce">Time's up!</div>
              )}
              <div className="flex gap-4 mb-8">
                <button onClick={() => setTimerDuration(5 * 60)} className="px-4 py-2 bg-neutral-200 dark:bg-neutral-800 rounded-lg text-sm">5m</button>
                <button onClick={() => setTimerDuration(15 * 60)} className="px-4 py-2 bg-neutral-200 dark:bg-neutral-800 rounded-lg text-sm">15m</button>
                <button onClick={() => setTimerDuration(25 * 60)} className="px-4 py-2 bg-neutral-200 dark:bg-neutral-800 rounded-lg text-sm">25m</button>
              </div>
              <div className="flex gap-6">
                <button 
                  onClick={() => {
                  ...
                  className="w-16 h-16 rounded-full bg-blue-600 hover:bg-blue-500 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-105"
                >
                  {timerRunning ? <Square className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 ml-1 fill-current" />}
                </button>
                <button 
                  onClick={() => { setTimerRunning(false); setTimerDuration(25 * 60); }}
                  className="w-16 h-16 rounded-full bg-neutral-200 dark:bg-neutral-800 hover:bg-neutral-300 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-300 flex items-center justify-center transition-transform hover:scale-105"
                >
                  <RotateCcw className="w-6 h-6" />
                </button>
